...
cppflags_gnu_error_handling = ['-Wall' ,'-Wextra' ,'-pedantic' ,'-std=c++17' ,'-O2' ,'-Wshadow' ,'-Wformat=2' ,'-Wfloat-equal' ,'-Wconversion','-Wcast-qual' ,'-Wcast-align' ,'-D_GLIBCXX_DEBUG' ,'-D_GLIBCXX_DEBUG_PEDANTIC' ,'-D_FORTIFY_SOURCE=2' ,'-fsanitize=address' ,'-fsanitize=undefined' ,'-fno-sanitize-recover' ,'-fstack-protector']
# cppflags = cppflags_ms + cppflags_gnu + cppflags_gnu_error_handling
cppflags = cppflags_gnu_error_handling
...
env = Environment(CPPPATH=include, LIBS=libs, LINK = 'ld.lld', CXX = 'clang++', CXXCOMSTR = "Compiling $SOURCE", LINKCOMSTR = "Linking $TARGET", CCFLAGS = cppflags)
...
env.Program('open-yorg', sources, CPPFLAGS = cppflags)